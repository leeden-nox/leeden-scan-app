...
export const ErrorPrinter = ( error ) => {
  ...
  notification.error({ message: 'Error', description: msg, placement: 'bottomRight'});
...
export const switchBranch = (code) => {
  let branches = JSON.parse(localStorage.getItem(LocalStorage.MODULE_GRANT)) || null;
...
export const manageSessionBySwitchBranch = (select) => {
  ...
  sessionStorage.setItem(SessionStorage.DIVISION_BRANCH_NAME, select.DivisionBranchName)
  sessionStorage.removeItem(SessionStorage.USER_DIMENSION);
  sessionStorage.removeItem(SessionStorage.DIMENSION_CONFIG);
}
